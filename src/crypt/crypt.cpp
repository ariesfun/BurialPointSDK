...
namespace buried
{
    ...
    void AESImpl::Init(const char* key, size_t key_size)
    {
        ...
        mbedtls_cipher_setup(&encrypt_ctx_, mbedtls_cipher_info_from_type(MBEDTLS_CIPHER_AES_256_CBC));
        mbedtls_cipher_set_padding_mode(&encrypt_ctx_, MBEDTLS_PADDING_PKCS7);
        mbedtls_cipher_setkey(&encrypt_ctx_, reinterpret_cast<const unsigned char*>(key), key_size * 8, MBEDTLS_ENCRYPT);
